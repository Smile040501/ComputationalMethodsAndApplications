...
class Polynomial:
    ...
    @handleError
    def bestFitFunction(self, f, func_str, x, y, n, shouldPlot=True):
        """
    ...
    @handleError
    def printRoots(self, eps=1e-3, only_real=False):
        """
        ...
        def hasConverged(roots):
            ...
            """
            for root in roots:
                if only_real and (self[root] / der[root]) > eps:
                    return False
                elif not only_real and (self[root] / der[root]).real > eps:
                    return False
            return True
...
def computeZerosFunction(fun, func_str, a, b, eps):
    """
    Approximates all the zeros of function f in [a, b]
    """
    # Computes the polynomial of degree n that best approximates the function in [a, b]
    p = Polynomial([])
    n = 17  # The degree of the polynomial to approximate the function
    p = p.bestFitFunction(fun, func_str, a, b, n, shouldPlot=False)

    # Computing the roots of the function polynomial
    roots = p.printRoots(eps, only_real=True)

    # Printing the roots
    print(f"The roots of {func_str} in [{a}, {b}] are:")
    for root in roots:
...
if __name__ == "__main__":
    ...
    inp_fun_str = "sin(x)"
    computeZerosFunction(inp_fun, inp_fun_str, 0, 10, 1e-3)
